...
use backend::AsyncOpenaiBackend;
use cli::*;
...
trait Backend {
    async fn add(&mut self, opts: &AddOpts) -> anyhow::Result<()>;
}
...
pub fn get_callbacks() -> ReplCallBacks {
    ...
    callbacks.insert("add".to_string(), cli::add);
    callbacks
}
...
impl ReplContext {
    pub fn new() -> Self {
        ...
        let mut backend = AsyncOpenaiBackend::new();
        thread::Builder::new()
